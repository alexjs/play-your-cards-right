...
newCard = function(req){
  ...
  new_sites = positions.filter(function(v) {
    return ( session.cardsPlayed.indexOf(v) == -1 )
  });
...
showIncorrectAnswerSame = function(req){
  req.session.message = { 'class': 'incorrect', 'text': 'You get nothing for a pair, not in this game!' };
}
